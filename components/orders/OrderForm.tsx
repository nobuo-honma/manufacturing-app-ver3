...
export default function OrderForm({ initialData, onSaved, onCancel }: Props) {
  const [customers, setCustomers] = useState<Customer[]>([])
  const [products, setProducts] = useState<Product[]>([])
  const [search, setSearch] = useState('')
  const [form, setForm] = useState({
    desired_ship_date: initialData?.desired_ship_date?.slice(0, 10) || '', 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      alert(`受注保存に失敗しました: ${error.message}`)
    }
  }

  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 440px) 1fr', gap: '20px', alignItems: 'start' }}>
      {/* ── 左：入力フォーム ── */}
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', background: 'var(--surface-2)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)' }}>
            {initialData ? '受注情報の編集' : '受注情報の入力'}
          </span>
        </div>

        <div style={{ padding: '24px' }}>
          {/* 登録完了 */}
          {submitted && !initialData && (
            <div style={{
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
              {onCancel && (
                <button type="button" onClick={onCancel} className="btn-secondary" style={{ flex: 1, padding: '13px 0', borderRadius: '999px' }}>
                  キャンセル
                </button>
              )}
              <button type="submit" disabled={saving} className="btn-submit"
                style={{ flex: 1, margin: 0 }}>
                {saving ? '保存中...' : initialData ? '更新する' : '受注を登録する'}
              </button>
            </div>
          </form>
        </div>
